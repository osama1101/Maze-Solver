...
public class MazeApp extends JFrame implements ActionListener {
	...
	if (mazeLoaded && !solver.isSolved()) {
	    solver.step();
	    if (solver.isSolved() || !solver.isSolvable()) {
		startButton();
		timer.stop();
	    }
	}
	*/
	...
	if(solverType==MazeSolver.STACK){
	    solver = new MazeSolver(this.maze,new MyStack<Square>());
	}
	else
	    solver = new MazeSolver(this.maze,new MyQueue<Square>());
    }
       ...
       if (mazeLoaded) {  // leave blank until first maze is loaded
		 ...
		 if(first){
		     ...
		     textPath += "-->";
                ...
                textPath += "["+sq.getRow()+","+sq.getColumn()+"]";
             }
